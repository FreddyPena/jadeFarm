...
public class VwUnidad extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        tbFamilia = new com.jadesoft.jadelib.estandar.tablas.TableStandar<InvUnidad>();
    ...
    private void tbFamiliaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbFamiliaMouseClicked
        if (evt.getClickCount() == 2) {
            if (!consultation) {
                ...
                this.unidad = tbFamilia.getSelectedElement();
                dispose();
            }
        }
...
//        }
    ...
    private com.jadesoft.jadelib.estandar.tablas.TableStandar<InvUnidad> tbFamilia;
    ...
    private void initComp() {
        ...
        tbFamilia.doLayout();
        tbFamilia.setModelStandar(new ModelUnidad());
    ...
    private void setToolBarAndPanel(int panel, boolean b) {
        ...
        btNew.setEnabled(b);
        btEdit.setEnabled(b);
        b = jtpStandard.getSelectedIndex() == 0 && this.unidad != null;
    ...
    private void delete() {
        if (this.unidad != null) {
            ...
            try {
                FactoryObject.getInstance().getUnidadController().delete(this.unidad.getCodigo());
            ...
            tbFamilia.removeElement(this.unidad);
            clean();
            setToolBarAndPanel(1, true);
            fireTableDataChanged();
        }
    ...
    private void save() {
        ...
        int men = JOptionPane.showConfirmDialog(this, this.unidad == null ? "Desea guardar el nuevo registro?"
        ...
        InvUnidad copyElement = this.unidad;
        if (this.unidad == null) {
            this.unidad = new InvUnidad();
        }

        this.unidad.setDescripcion(jtDescripcion.getText());
        this.unidad.setIdentificador(jtIdentificador.getText());

        if (copyElement == null || !this.unidad.equals(copyElement)) {
            try {
                FactoryObject.getInstance().getUnidadController().saveOrUpdate(this.unidad);
            } catch (BussinessException ex) {
            ...
            tbFamilia.addElement(this.unidad);
        }
