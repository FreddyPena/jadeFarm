...
import com.jsft.inv.tablemodel.ModeloUnidad;
 ...
 * @author yiyi
 */
public class DgUnidad extends javax.swing.JDialog {

    public DgUnidad(javax.swing.JDialog parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    public DgUnidad(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        setLocationRelativeTo(parent);
        initComp();
    ...
    private void initComponents() {
        ...
        tbUnidad = new com.jadesoft.jadelib.estandar.tablas.TableStandar<Unidad>();
        ...
        jXHeader1.setDescription("Registro y consulta de Unidades");
    ...
    private void tbUnidadMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbUnidadMouseClicked
        objectDisplacement.setLista(tbUnidad.getElements());
        if (evt.getClickCount() == 2) {
            setElement(tbUnidad.getSelectedElement());
            if (!consultation) {
    ...
    private void jtFiltroKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_jtFiltroKeyReleased
        General.filtroTextField(cbFiltro.getSelectedItem(), jtFiltro, tbUnidad);
    }//GEN-LAST:event_jtFiltroKeyReleased

    public static void main(String[] args) {
        new DgUnidad((JDialog) null, true).setVisible(true);
    }
    ...
    private com.jadesoft.jadelib.estandar.tablas.TableStandar<Unidad> tbUnidad;
    // End of variables declaration//GEN-END:variables
    private KeyEventDespachador eventDispatcher;
    private Unidad unidad;
    private DesplazamientoObject<Unidad> objectDisplacement;
    private boolean consultation;
    private boolean changeDisplacement = true;
    

    protected void setColumnaWith(int Column, int width) {
        tbUnidad.getColumnModel().getColumn(Column).setPreferredWidth(width);
    }
    ...
    private void initComp() {
        ...
        tbUnidad.doLayout();
        tbUnidad.setModelStandar(new ModeloUnidad());
        tbUnidad.setElements(ManejoUnidad.getInstance().findAll());
    ...
    private void edit() {
        if (tbUnidad.getSelectedElement() != null) {
            setDisplacement(true);
            objectDisplacement.setLista(tbUnidad.getElements());
            setElement(tbUnidad.getSelectedElement());
            setTablePanel(0, false);
    ...
    private void setToolBar(boolean b) {
        ...
        b = jtpStandard.getSelectedIndex() == 0 && unidad != null;
    ...
    private void delete() {
        if (unidad != null) {
            ...
            if (men == JOptionPane.YES_OPTION) {
                if (ManejoUnidad.getInstance().findDelete(unidad) != null) {
                    JOptionPane.showMessageDialog(null, "Existen artículos relacionados a este Unidad. No se puede borrar!", "Aviso",
                            JOptionPane.WARNING_MESSAGE);
                    return;
                }
                ManejoUnidad.getInstance().delete(unidad);
            }
            clean();
            setTablePanel(0, false);
            setToolBar(false);
    ...
    private void select() {
        if (changeDisplacement) {
            objectDisplacement = new DesplazamientoObject<>(tbUnidad.getElements());
    ...
    private void save() {
        ...
        int men = JOptionPane.showConfirmDialog(this, unidad == null ? "Desea guardar el nuevo registro?"
        ...
        unidad.setDescripcion(jtDescripcion.getText());
        unidad.setIdentificador(jtIdentificador.getText());
        ManejoUnidad.getInstance().create(unidad);
...
//        });
    ...
    private void setElement(Unidad unidad) {
        selectRow();
        if (unidad != null) {
            this.unidad = unidad;
            jtIdentificador.setText(unidad.getIdentificador());
            jtDescripcion.setText(unidad.getDescripcion());
            editableComponent(false);
            //  objectDisplacement.setRegistroActual(unidad);           
        }
    }
    ...
    private void selectRow() {
        ...
        if (unidad != null) {
            ...
            if (jtpStandard.getSelectedIndex() == 1) {
                int row = 0;
                for (Unidad f : tbUnidad.getElements()) {
                    if (f.equals(unidad)) {
                        tbUnidad.changeSelection(row, 1, false, false);
                        return;
                    }
                    row++;
                }
        ...
        if (jtpStandard.getSelectedIndex() == 1 && tbUnidad.getSelectedRow() == -1) {
            tbUnidad.changeSelection(0, 1, false, false);
            objectDisplacement.setLista(tbUnidad.getElements());
        }
    }
